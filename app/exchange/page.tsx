...
const stepInfos = [
  { label: "Обработка", desc: "⏳ Ваша заявка обрабатывается...", color: "#999" },
  { label: "Проверка", desc: "🔍 Проверяем данные и совершаем обмен...", color: "#9dc21b" },
  { label: "Завершено", desc: "✅ Обмен успешно завершён!", color: "text-green-400" }
] as const
...
const steps: ("processing" | "verifying" | "completed")[] = ["processing", "verifying", "completed"]
...
const ExchangePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const parsed: ExchangeRequest = JSON.parse(data)
    setRequest(parsed)
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (i < steps.length) {
        setStatus(steps[i])
        setActiveStep(i)
        if (i === 2) setTimeout(() => setShowGlow(true), 900)
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-black text-white">
      ...
      <main className="flex-1 container mx-auto px-4 py-10 max-w-3xl">
          ...
          >
            ...
            <motion.div
              ...
              variants={{
                ...
                visible: { opacity: 1, y: 0, transition: { staggerChildren: 0.045 } }
              }}
              className="space-y-3 text-sm text-[#ccc] mb-8"
            >
              {[
                { label: "Тип операции", value: request.type === "buy" ? "Купить" : "Продать" },
                { label: "Отдаёте", value: `${request.giveAmount} ₽ (${request.giveBank})` },
                { label: "Получаете", value: `${request.receiveAmount} ${request.receiveCrypto}` },
                { label: "Карта", value: request.cardNumber },
                { label: "Кошелёк", value: request.wallet },
                { label: "Курс", value: request.rate ? `${request.rate.toFixed(2)} ₽` : "—" },
                { label: "Создано", value: new Date(request.createdAt).toLocaleString() }
              ].map((item, idx) => (
                <motion.p
                  key={item.label}
            ...
            <div className="mb-7">
              ...
              <div className="relative flex justify-between text-xs uppercase tracking-widest text-[#777] mb-2 z-10">
                {stepInfos.map((step, idx) => (
                  <motion.div
                    ...
                    initial={false}
                    animate={{ color: activeStep === idx ? "#9dc21b" : activeStep > idx ? "#6fb550" : "#555" }}
                  >
                    <motion.div
                      animate={{ scale: activeStep === idx ? 1.15 : 1, filter: activeStep === idx ? "drop-shadow(0 0 12px #befc3a88)" : "none" }}
              ...
              <div className="relative w-full h-3 rounded-full overflow-hidden bg-[#232b1c] shadow-sm mb-2">
                ...
                <motion.div
                  ...
                  animate={{ opacity: status === "completed" ? 0.18 : 0, background: status === "completed" ? "repeating-linear-gradient(-45deg,#bcf52740 0 10px,#477c1360 10px 20px)" : "transparent" }}
            ...
            <div className="text-center mt-10 min-h-[40px] flex justify-center items-center relative z-10">
              <AnimatePresence mode="wait">
                ...
                >
                  {status === "processing" && <span className="text-[#999] text-lg block">{stepInfos[0].desc}</span>}
                  {status === "verifying" && <span className="text-[#9dc21b] text-lg block drop-shadow">{stepInfos[1].desc}</span>}
                  {status === "completed" && <span className="text-green-400 font-semibold text-lg block drop-shadow-lg animate-glow">{stepInfos[2].desc}</span>}
                </motion.div>
              </AnimatePresence>
            </div>
          </motion.div>
        ...
        >
          <motion.button
            whileHover={{ scale: 1.06, boxShadow: "0 0 0 0 #bcf52788,0 0 10px 2px #9dc21b42" }}
            whileTap={{ scale: 0.94, backgroundColor: "#bbdc60" }}
            onClick={() => router.push("/")}
...
const ExchangePageWrapper: React.FC = () => (
  <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-white bg-black">Загрузка...</div>}>
    <ExchangePage />
  </Suspense>
)

export default ExchangePageWrapper
